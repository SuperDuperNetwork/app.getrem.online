          ...
          <CardContent className="space-y-3">
            <div className="flex justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="ghost" size="sm" onClick={() => navigator.clipboard.writeText(collectionId)}>
...
client.upsert("${collectionId}", vectors=[
    {"id": "vec1", "values": [0.1] * 384, "metadata": {"text": "hello"}},
])

# Query
results = client.query("${collectionId}", vector=[0.1] * 384, top_k=10)`}</code>
            </pre>
          </div>
          <div>
            <h4 className="mb-2 text-sm font-medium">cURL</h4>
            <pre className="rounded-md bg-muted p-4 text-sm">
              <code>{`curl -X POST https://api.getrem.online/v1/collections/${collectionId}/vectors/query \\
  -H "X-API-Key: rem_xxx" \\
